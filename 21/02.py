...
allergen_names = {}
while len(allergen_names.keys()) < 8:
    for cand in allergen_candidates.keys():
        if not (allergen_candidates[cand]):
            continue
        if len(allergen_candidates[cand]) == 1:
            allergen_names[cand] = allergen_candidates[cand].pop()
            for cand_r in allergen_candidates.keys():
                allergen_candidates[cand_r].discard(allergen_names[cand])

out = ""
for allergen in sorted(list(allergen_names.keys())):
    print("{:<10} {}".format(allergen+":", allergen_names[allergen]))
    out += allergen_names[allergen] + ","
out  = out[:-1]
